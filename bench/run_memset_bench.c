...
void *run_memset_bench(struct bench_conf *c) {
  ...
  if (c->flag_random) {
    ...
    if (c->flag_nodb == 0) {
      sprintf(cmd, "INSERT INTO %s VALUES(%d, %d, %d, %d, %f, %f, %d, %d)",
              c->benchtitle, c->nothreads, c->curthread, c->size, c->loops, dt, bw, k, c->flag_random);
