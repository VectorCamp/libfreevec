...
#define BENCHFUNC run_memset_bench
#define SCALARFUNC memset
#define VECTORFUNC vec_memset
...
void prepare_dbtable(struct bench_conf *conf) {
  ...
  strcat(cmd, " (nothreads INTEGER, curthread INTEGER, size INTEGER, loops INTEGER, dt DOUBLE, bw DOUBLE, al_src INTEGER, random INTEGER)");
...
int main(int argc, char *argv[]) {
  ...
  conf.testdata = mmap(0, conf.testdatasize, PROT_READ | PROT_WRITE, MAP_PRIVATE, randomdata, 0);
