...
void *run_cos_test(struct bench_conf *c) {
    ...
    for (l = 0; l < c->loops; l++) {
        x[l] = M_PI_2 * ((double)rand() / (double)RAND_MAX);
    }
    ...
    printf("Glibc      : %12.2f calculations of cos()/sec\n", mcalc);
    ...
    dt = (t1 - t0) / ticspersec - dt0;
    mcalc = (double)(c->loops) / (dt);
    printf("Libfreevec : %12.2f calculations of cos()/sec\n", mcalc);
