...
void *run_cosf_test(struct bench_conf *c) {
  ...
  float *x = malloc(XSIZE*sizeof(float));
  if (x == NULL) exit(1);
  ...
  mcalc = (double)(XSIZE) / (dt);
  printf("Glibc      : %10.2f calculations of cosf()/sec\n", mcalc);
  ...
  dt = (t1 - t0) / ticspersec - dt0;
  mcalc = (double)(XSIZE) / (dt);
  printf("Libfreevec : %10.2f calculations of cosf()/sec\n", mcalc);
  ...
  printf("vcosf fail/tot = %d/%d, maxdiff = %2.7f\n", counter1, XSIZE, maxdiff1);
