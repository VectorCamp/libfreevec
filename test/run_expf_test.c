...
void *run_expf_test(struct bench_conf *c) {
    ...
    uint64_t cyc1, cyc2;
    float r1, r2, r3, diff1, diff2, maxdiff1 = 0.0, maxdiff2 = 0.0, rel, maxrel;
    ...
    printf("Glibc      : %12.2f calculations of expf()/sec\n", mcalc);
    ...
    dt = (t1 - t0) / ticspersec - dt0;
    mcalc = (double)(c->loops) / (dt);
    printf("Libfreevec : %12.2f calculations of expf()/sec\n", mcalc);
    ...
    printf("vec_expf fail/tot = %d/%d, maxdiff = %2.7f, maxrel = %5.7f\n", counter1, c->loops, maxdiff1, maxrel);
